...
@click.option("--no-pdf", is_flag=True, help="Skip PDF export; output HTML only. (PDF is generated by default via Chrome headless.)")
...
def apply_command(url, no_cover_letter, no_pdf, model, output_dir, open_browser, notes, track, no_report):
    """Generate a tailored CV and cover letter for a job listing.

    URL is optional — omit to pick interactively from listings.csv.
    Exports PDFs by default (via Chrome headless); add --no-pdf for HTML only.
    """
    pdf = not no_pdf
