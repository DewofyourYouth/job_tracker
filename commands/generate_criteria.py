...
EVAL_SYSTEM_PROMPT_PATH = Path("data/llm-eval-system.md")
...
# ---------------------------------------------------------------------------
# LLM eval system prompt generation (no API call — pure template render)
# ---------------------------------------------------------------------------

def _build_location_context(profile: dict) -> str:
    """Derive a plain-English location/visa sentence from profile.yaml."""
    loc = profile.get("location", {})
    comp = profile.get("compensation", {})
    city = loc.get("city", "")
    country = loc.get("country", "")
    visa = loc.get("visa_status", "")
    flexibility = comp.get("location_flexibility", "")

    parts = []
    if city and country:
        parts.append(f"The candidate is based in {city}, {country}.")
    elif country:
        parts.append(f"The candidate is based in {country}.")
    if visa:
        parts.append(visa + ("." if not visa.endswith(".") else ""))
    if flexibility:
        parts.append(flexibility + ("." if not flexibility.endswith(".") else ""))
    return " ".join(parts)


def generate_eval_system_prompt(
    profile_text: str,
    output_path: Path = EVAL_SYSTEM_PROMPT_PATH,
    *,
    dry_run: bool = False,
) -> str:
    """
    Render prompts/llm_eval_system.md with candidate-specific context from
    profile.yaml and write the result to data/llm-eval-system.md.

    The output file is gitignored and human-readable. Edit it directly to
    tune how the LLM evaluates fit for your specific situation — rerun
    generate-criteria to regenerate from the template.
    """
    profile = yaml.safe_load(profile_text) or {}
    location_context = _build_location_context(profile)
    rendered = render_prompt("llm_eval_system.md", location_context=location_context)

    header = (
        "<!-- Generated by `job generate-criteria`. Safe to edit — rerun the command to regenerate. -->\n\n"
    )
...
def generate_criteria_command(
    ...
    generate_eval_system_prompt(profile_text, output_path=EVAL_SYSTEM_PROMPT_PATH)
    console.print(f"[green]✓[/] LLM eval system prompt written to [bold]{EVAL_SYSTEM_PROMPT_PATH}[/]")
    console.print(
        "  [dim]Review and edit this file to tune how the LLM evaluates fit for your situation.[/]"
    )
